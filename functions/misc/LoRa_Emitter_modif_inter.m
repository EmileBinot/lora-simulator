function [txSig,dataIn,interleaverOut,payload] = LoRa_Emitter_modif_inter(CR,SF,Pr_len,binary_data,whiteNoise,bypHamm,bypInter)
...
for i = 1: (length(hammingIn))
    hammingOut(:,i) = LoRa_Encode_Hamming(hammingIn(:,i)',CR,bypHamm);
end
...
if(bypInter==1)
    ...
    for i = 1 :length(interleaverIn)
       interleaverOut=[interleaverOut ; interleaverIn{i}.'];
    end 
else
    whiteOutBlock = reshape(whiteOut,CR+4,[])'; % vector to SF*x_blocks,CR+4 matrix
