...
for k = 1:length(c)
    ...
    transmittedIQ= transmittedIQ + rectangularPulse(0+((k-1)*T), T+((k-1)*T), ts).*exp(2i*pi*(B/(2*T).*ts+c(k)/T).*ts).*exp(-2i*pi*B.*ts);
...
figure(1)
plot(ts,real(transmittedIQ),ts,imag(transmittedIQ));

figure;
spectrogram(transmittedIQ,20,15,128,B,'yaxis');    % no idea how it's working
